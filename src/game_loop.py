...
def draw_board(dim, state,env):
    ...
    for i in range(dim+1, dim+4):
        for j in range(1, 2*dim+4):
            if state[0][matrix[i][j]]==0: window.addch(i, j, PATH_CHAR, curses.color_pair(255))
            else: window.addch(i, j, PIECE_CHAR, curses.color_pair((state[0][matrix[i][j]]-1)//TOKENS+1))
            if state[0][matrix[j][i]]==0: window.addch(j, i, PATH_CHAR, curses.color_pair(255))
            else: window.addch(j, i, PIECE_CHAR, curses.color_pair((state[0][matrix[j][i]]-1)//TOKENS+1))
    for i in range(dim+3, 2 * dim + 3):  # 1
        if state[1][0][i-dim-3]==0: window.addch(i, dim + 2, HOME_CHAR, curses.color_pair(1))
        else:  window.addch(i, dim + 2, PIECE_CHAR, curses.color_pair(1))
    for i in range(2, dim+2):  # 3
        if state[1][2][i-2]==0: window.addch(dim + 2, i, HOME_CHAR, curses.color_pair(3))
        else: window.addch(dim + 2, i, PIECE_CHAR, curses.color_pair(3))
    for i in range(2, dim+2):  # 2
        if state[1][1][i-2]==0: window.addch(i, dim + 2, HOME_CHAR, curses.color_pair(2))
        else: window.addch(i, dim + 2, PIECE_CHAR, curses.color_pair(2))
    for i in range(dim+2, 2 * dim + 3):  # 4
        if state[1][3][i-dim-3]==0: window.addch(dim + 2, i, HOME_CHAR, curses.color_pair(4))
        else: window.addch(dim + 2, i, PIECE_CHAR, curses.color_pair(4))
    window.addch(dim+2, dim+2, ' ')
